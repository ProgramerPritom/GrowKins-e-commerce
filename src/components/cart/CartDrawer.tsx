      ...
      {cartDrawerOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          >
            ...
            {/* Top Header */}
            <div className="p-6 border-b border-[#E8E0D2] flex items-center justify-between bg-white">
              ...
              <motion.button 
                whileTap={{ scale: 0.9 }}
                onClick={() => setCartDrawerOpen(false)}
                className="p-2 rounded-full hover:bg-[#F4EFE6] text-[#757169] hover:text-[#24221F] transition-colors"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {cartItems.length > 0 ? (
                <>
                    ...
                    <AnimatePresence>
                      {cartItems.map((item, idx) => (
                        <motion.div 
                          ...
                          className="flex gap-4 p-3.5 rounded-2xl bg-white border border-[#E8E0D2] shadow-2xs text-left"
                        >
                          <div className="w-20 h-20 rounded-xl overflow-hidden bg-[#FAF7F1] shrink-0 border border-[#E8E0D2]">
                            <img
                          ...
                          <div className="flex-1 flex flex-col justify-between">
                            <div>
                              <div className="flex items-start justify-between gap-2">
                                <h3 className="font-serif font-bold text-sm text-[#24221F] line-clamp-1">
                                  {item.product.name}
                                </h3>
                                <motion.button
                                  whileTap={{ scale: 0.85 }}
                                  onClick={() => removeFromCart(item.product.id)}
                                  className="text-[#A8A49C] hover:text-[#D96F58] p-1 transition-colors"
                                  title="Remove item"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                </motion.button>
                              </div>
                              <span className="text-[11px] font-mono text-[#757169] block">
                                {item.product.ageBadge}
                              </span>
                            </div>

                            <div className="flex items-center justify-between pt-2">
                              <div className="flex items-center border border-[#E8E0D2] rounded-full px-2 py-0.5 bg-[#FAF7F1]">
                                <motion.button
                                  whileTap={{ scale: 0.85 }}
                                  onClick={() => updateQuantity(item.product.id, -1)}
                                  className="w-5 h-5 flex items-center justify-center text-[#757169] hover:text-[#24221F]"
                                >
                  ...
                  {/* Gift Toggle Section */}
                  <div className="bg-[#FAF7F1] border border-[#E8E0D2] rounded-2xl p-4 text-left space-y-3">
                    <label className="flex items-center gap-2 text-xs font-semibold text-[#24221F] cursor-pointer">
                      ...
                      <span className="flex items-center gap-1.5">
                        <Gift className="w-3.5 h-3.5 text-[#F28F79]" /> This is a special gift (Gift packing included)
                      </span>
                    </label>
                    ...
                    {isGift && (
                      ...
                      >
                        <input
                          ...
                          className="w-full text-xs p-2.5 rounded-xl border border-[#D9D3C7] bg-white text-[#24221F]"
                        />
                        <textarea
                          placeholder="Gift card message (e.g. 'Happy Birthday Ryan! Lots of love from Khalamoni')"
                          value={giftMessage}
                  ...
                  {crossSellProduct && (
                    <div className="bg-white rounded-2xl p-3.5 border border-[#E8E0D2] text-left">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-[#A67E14] block mb-2">
                        Pairs Nicely With
                      </span>
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl overflow-hidden bg-[#FAF7F1] shrink-0">
                          <img
                        ...
                        <div className="flex-1">
                          <div className="text-xs font-bold text-[#24221F] line-clamp-1">{crossSellProduct.name}</div>
                          <div className="text-[11px] text-[#757169] font-sans">৳{crossSellProduct.price}</div>
                        </div>
                        <motion.button
                          whileTap={{ scale: 0.94 }}
                          onClick={() => addToCart(crossSellProduct, 1)}
                          className="px-3 py-1 rounded-full bg-[#FAF7F1] hover:bg-[#1C4CB8] hover:text-white border border-[#E8E0D2] text-xs font-semibold text-[#24221F] transition-colors"
                        >
                          + Add
                        </motion.button>
                      </div>
                ...
                <div className="text-center py-16 space-y-4">
                  ...
                  <p className="text-sm text-[#757169] max-w-xs mx-auto">
                  ...
                  <motion.button
                    ...
                    className="px-6 py-2.5 rounded-full bg-[#24221F] text-white hover:bg-[#1C4CB8] text-xs font-bold transition-all"
                  >
                    Explore Discoveries
                  </motion.button>
                </div>
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t border-[#E8E0D2] bg-white space-y-4">
                
                {/* Delivery Zone Toggle in Drawer */}
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-[#FAF7F1] border border-[#E8E0D2] text-xs">
                  <span className="text-[#6E6A63] font-medium">Delivery Zone:</span>
                ...
                {/* Price Calculations */}
                <div className="space-y-1.5 text-xs text-[#6E6A63]">
                  <div className="flex justify-between">
                    <span>{t.cart.subtotal}</span>
                    <span className="font-semibold text-[#24221F] font-sans">৳{subtotal}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t.cart.deliveryFee} ({deliveryDetails.deliveryZone === 'inside-dhaka' ? 'Inside Dhaka' : 'Outside Dhaka'})</span>
                    <span>{deliveryFee === 0 ? <strong className="text-[#4F7A5E]">{t.cart.free}</strong> : `৳${deliveryFee}`}</span>
                  </div>
                  <div className="flex justify-between pt-2 border-t border-[#E8E0D2] text-sm font-bold text-[#24221F]">
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-[#FAF7F1] border border-[#E8E0D2] text-xs text-[#24221F]">
                  <PackageCheck className="w-4 h-4 text-[#A67E14] shrink-0" />
                  <div>
                    <span className="font-bold">Cash on Delivery Across Bangladesh</span>
                    <p className="text-[11px] text-[#757169]">Zero advance payment required. Inspect your parcel at delivery before paying.</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <motion.button
                    ...
                    className="w-full py-3.5 rounded-full bg-[#24221F] text-[#FAF7F1] hover:bg-[#1C4CB8] text-sm font-bold flex items-center justify-center gap-2 shadow-md transition-colors cursor-pointer"
                  >
                  ...
                  <div className="flex items-center justify-between text-xs text-[#757169] px-1">
                    <button
                      ...
                      className="hover:text-[#24221F] underline"
                    >
                      View full bag
                    </button>
                    <button
